...
int main(){
    ...
    std::vector<double> X3, Y3, eX3, eY3;
    std::ifstream in;

    in.open("input/ajustes2.txt");
    while(!in.eof()){
        in >> x >> y;
        // 1.52249*x + 0.959
        // 0.00029 + 0.066
    ...
    TGraphErrors* g3 = new TGraphErrors(X3.size(), X3.data(), Y3.data(), eX3.data(), eY3.data());
    ...
    TF1 * f = new TF1("f"," [0]*TMath::Gaus(x, [1], [2]) + [3]*TMath::Erfc( (x - [1]) / TMath::Sqrt(2*[2]) ) ",0,1500);
    TF1 * f2 = new TF1("f2"," [0]*TMath::Gaus(x, [1], [2]) + [3]*TMath::Erfc( ([1] - x) / TMath::Sqrt(2*[2]) ) ",0,1500);
    TF1 * f3 = new TF1("f3"," [0]*TMath::Gaus(x, [1], [2]) + [3]*TMath::Erfc( (x - [1]) / TMath::Sqrt(2*[2]) ) ",0,1500);
    ...
    f->SetParameters(30,620,70,30);
    f2->SetParameters(80,145,15,30);
    f3->SetParameters(25,720,70,15);

    g1->Fit("f");
    g2->Fit("f2");
    g3->Fit("f3");

    TPaveText* pt1 = new TPaveText(450, 130, 550, 220, "user");
    TPaveText* pt2 = new TPaveText(100, 180, 200, 250, "user");
    TPaveText* pt3 = new TPaveText(800, 180, 990, 250, "user");
    ...
    pt1->AddText(Form("Compton Edge 1 Fit"));
    pt1->AddText(Form("#alpha = %.1f %c %.1f", f->GetParameter(0), 0xB1, f->GetParError(0)));
    pt1->AddText(Form("#beta = %.1f %c %.1f", f->GetParameter(3), 0xB1, f->GetParError(3)));
    pt1->AddText(Form("#sigma = (%.2f %c %.2f)#upoint10^{2}", f->GetParameter(2)*0.01, 0xB1, f->GetParError(2)*0.01));
    ...
    pt3->AddText(Form("Compton Edge 2 Fit"));
    pt3->AddText(Form("#alpha = %.1f %c %.1f", f3->GetParameter(0), 0xB1, f3->GetParError(0)));
    pt3->AddText(Form("#beta = %.1f %c %.1f", f3->GetParameter(3), 0xB1, f3->GetParError(3)));
    pt3->AddText(Form("#sigma = (%.1f %c %.1f)#upoint10^{2}", f3->GetParameter(2)*0.01, 0xB1, f3->GetParError(2)*0.01));
    pt3->AddText(Form("Ec = %.0f %c %.0f", f3->GetParameter(1), 0xB1, f3->GetParError(1)));

    pt3->AddText(Form("#chi^{2} = %.2f", f3->GetChisquare()));
    pt3->AddText(Form("Number of points = %ld", X3.size() ));
